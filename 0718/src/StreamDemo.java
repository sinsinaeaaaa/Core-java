/*
 * Stream API
 * 1. Stream? : 배열이나 컬렉션과 같이 여러 개의 데이터를 일관되게 처리하기 위해서 추가된 문법
 * 2. Data를 Stream으로 처리하기 위해서는 먼저 Stream객체를 생성해야 한다.
 * 3. Stream API의 3가지 종류
 * 	1) Stream 생성
 *  2) 생성된 Stream을 가공하는 중간연산
 *  3) 최종적으로 결과를 만드는 최종 연산
 * 4. 연산의 결과가 Stream으로 리턴되면 중간연산이라고 한다. 반면, 연산의 결과가 Stream이 아닌 형태로 나타나면 최종 연산이라고 한다.
 * 5. 중간연산 결과가 Stream으로 리턴이 되기 때문에, Stream연산을 계속 이어서 chain 형태로 이옹할 수 있다. 
 * 6. But, 최종연산의 결과는 Stream이 아니기 때문에 일단 최종 연산을 수행하면 다시 다른 스트링 연산을 수행할 수 없다.
...
public class StreamDemo {
	...
	public static void main(String[] args) {
		...
		Stream <Integer> s1 = Stream.of(intArray); //of 를 사용한다.
		
		String [] strArray = {"사과","배","딸기","복숭아","레몬","망고","수박"};
		Stream<String> s2 = Stream.of(strArray); //of 를 사용한다.
		
		//Arrays로 부터 Stream 생성하기
		Stream<Integer> s3 = Arrays.stream(intArray);
		Stream<String> s4 = Arrays.stream(strArray);
		
		//Collection으로 부터 Stream 생성하기
		...
		// 난수를 발생하여 
		DoubleStream ds1 = new Random().doubles(15L, 1.0, 10.0); // 1.0 부터 10.0 사이에서 15개 랜덤숫자 뽑아내기
		IntStream is1 = new Random().ints(5,1,10) ; // 1부터 10까지 정수 랜덤하게 5개 뽑아내기
